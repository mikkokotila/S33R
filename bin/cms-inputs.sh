...
taxonomy(){
	...
	echo -e "11~Bouncerate~"$SW_ENGAGEMENTS"~Similarweb"
	echo -e "12~% share of search visits~"$ALEXA_SEARCHVISITS"~Alexa"
	echo -e "13~% share of search visits~"$SW_TRAFFICSOURCES"~Similarweb"
	echo -e "14~top5 keywords' share of all search traffic~"$ALEXA_TOPKEYWORDS"~Alexa (computed)"
	echo -e "15~organic search traffic's share of all search~"$SW_ORGANICSEARCH"~Similarweb"
	echo -e "16~paid search traffic's share of all search~"$SW_PAIDSEARCH"~Similarweb"
	echo -e "17~Votes~"$WOT_VOTES"~Web of Trust"
	echo -e "18~Trust~"$WOT_TRUST"~Web of Trust"
	echo -e "19~Childsafety~"$WOT_CHILDSAFETY"~Web of Trust"
	echo -e "20~Google Analytics ID~"$GA"~W3bin"
	echo -e "23~top5 inbound traffic share of all referrals~"$TOP5_UPSTREAM"~Alexa"
	echo -e "24~the top inbound traffic source~"$SITES_UPSTREAM"~Alexa"
	echo -e "25~Loadspeed~"$ALEXA_LOADSPEED"~Alexa"
	echo -e "26~Registrant email~"$REGISTRANT_EMAIL"~Whois"
}
