...
whois_privacy(){
	...
	if [[ -n "$TRAFFIC_TREND" ]]; then
		TRAFFIC_TREND=$(echo -e "scale=2; ($SW_2016_07_01 - $SW_2016_03_01) / $SW_2016_03_01" | bc -l); echo -e "TRAFFIC_TREND=$TRAFFIC_TREND" >> env.bash
	fi

	if [[ -n "$SW_PAGEVIEWS" && -n "SW_2016_04_01" ]]; then
